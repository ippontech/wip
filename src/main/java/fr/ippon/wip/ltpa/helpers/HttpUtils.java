...
public class HttpUtils {
    ...
    public static byte[] base64Decode(String data) throws Base64DecodeException {
        if (data.length() == 0) return new byte[0];
        Vector<Byte> dest = new Vector<Byte>(data.length());
        ...
        for (int i = 0; i < ret.length; i++)
            ret[i] = dest.get(i).byteValue();

        return ret;
    }

    public static boolean isBase64Encoded(String sBase64) {
