...
public class ClippingURIResolver implements URIResolver {
    ...
    public Source resolve(String href, String base) throws TransformerException {
        if (config.getClippingType().equals("xslt")) {
            return new StreamSource(new ByteArrayInputStream(config.getXsltClipping().getBytes()));
        }
        InputStream defaultStream = getClass().getClassLoader().getResourceAsStream("xslt/default-clipping.xslt");
        return new StreamSource(defaultStream);
    }
}
